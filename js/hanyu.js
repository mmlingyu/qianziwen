...
function createImage() {
    ...
    $.each(list[pageIndex], function (index, item) {
        html += '<div class="image-hanItem" data-voice=' + item.audio + '>';
        if (item.type == 2) {
            html += "<div class='image-top'>" + item.name.slice(0, 1) + "<span>" + item.name.slice(1, 2) + "<i>" + item.han + "</i></span></div>"
        } else {
            html += "<div class='image-top'><span>" + item.name.slice(0, 1) + "<i>" + item.han + "</i></span>" + item.name.slice(1, item.name.length) + "</div>"
        }
...
function lunsss() {
    var items = document.getElementsByClassName('image-hanItem');
    speak(items[itemIndex].getAttribute('data-voice'));
    $('.image-hanItem').eq(itemIndex).addClass('active');
    itemIndex++;
    if (itemIndex >= 8) {
        itemIndex = 0;
        showbig = true;
...
function createSwiper() {
    ...
    $.each(list[pageIndex], function (index, item) {
        ...
        html += '<span class="name han">'
        if (item.type == 2) {
            html += item.name.slice(0, 1) + "<span class='red'>" + item.name.slice(1, 2) + "<i>" + item.han + "</i></span>"
        } else {
            html += "<span class='red'>" + item.name.slice(0, 1) + "<i>" + item.han + "</i></span>" + item.name.slice(1, item.name.length)
        }
